...
**前提**：已完成域名购买 + DNS 解析 + SSL 证书配置（参考第 6 步）。

1. 用[微信开发者工具](https://developers.weixin.qq.com/miniprogram/dev/devtools/download.html)打开 `miniapp/` 目录
2. 确认 `utils/config.js` 中 `PROD_DOMAIN` 已改为 `https://你的域名.com`
3. 点击工具栏 **「上传」**，版本号填 `1.0.x`
4. 登录 [微信公众平台](https://mp.weixin.qq.com/) → 管理 → 版本管理
5. 在「体验版」中设置体验成员 → 生成二维码分享

**重要**：微信小程序真机环境强制 HTTPS，必须先完成域名 + SSL 配置。

详见 [DEPLOYMENT-LOG.md](DEPLOYMENT-LOG.md) 第 4 节完整步骤。
