...
export default function SupplierPaymentReportPage() {
  ...
  const handleExport = () => {
    ...
    const headers = ['供应商', '采购次数', '总金额'];
    const rows = data.map((d: any) => [d.name, d.count, d.totalAmount]);
    downloadCSV(headers, rows, `供应商货款报表_${ts}.csv`);
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
        <h2 style={{ margin: 0, fontSize: 20, fontWeight: 600 }}>供应商货款报表</h2>
        <Space>
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col xs={24} sm={12}>
          <Card hoverable size="small">
            <Statistic title="付款总金额" value={totalAmount} precision={2} prefix="¥" valueStyle={{ fontWeight: 600 }} />
          </Card>
        </Col>
        <Col xs={24} sm={12}>
          <Card hoverable size="small">
            <Statistic title="采购批次数" value={totalCount} valueStyle={{ fontWeight: 600 }} />
          </Card>
        </Col>
      </Row>

      <Spin spinning={loading}>
        <Table dataSource={data} rowKey="supplierId" size="small"
          columns={[
            { title: '供应商', dataIndex: 'name', width: 200 },
            { title: '采购次数', dataIndex: 'count', width: 120, sorter: (a: any, b: any) => a.count - b.count },
            { title: '总金额', dataIndex: 'totalAmount', width: 180, render: (v: number) => `¥${Number(v).toLocaleString()}`, sorter: (a: any, b: any) => a.totalAmount - b.totalAmount },
          ]}
